#ifndef LAYER_CONVOLUTIONALSGD_H_
#define LAYER_CONVOLUTIONALSGD_H_
...
///
    ...
    private:
        ...
        Parameters *init_param_object_ptr;
        ConvolutionalParameters *current_parameters = nullptr;
    ...
    public:
        ...
        Convolutional(Convolutional &layer) = default;

        void init(const Scalar &mu, const Scalar &sigma, RNG &rng) {
            layer_initial_parameters_ptr = new ConvolutionalParameters();
            ConvolutionalParameters *convolutional_initial_parameters_pointer = dynamic_cast<ConvolutionalParameters *>(layer_initial_parameters_ptr);

            // Set data dimension
            const int filter_data_size = m_dim.in_channels * m_dim.out_channels *
                                         m_dim.filter_rows * m_dim.filter_cols;

            //convolutional_initial_parameters_pointer->m_filter_data = new Vector();
            convolutional_initial_parameters_pointer->m_filter_data.resize(filter_data_size);
            m_df_data.resize(filter_data_size);
            // Random initialization of filter parameters
            internal::set_normal_random(convolutional_initial_parameters_pointer->m_filter_data.data(),
                                        filter_data_size, rng, mu,
                                        sigma);
            // Bias term
            //convolutional_initial_parameters_pointer->m_bias = new Vector();
            convolutional_initial_parameters_pointer->m_bias.resize(m_dim.out_channels);
            m_db.resize(m_dim.out_channels);
            internal::set_normal_random(convolutional_initial_parameters_pointer->m_bias.data(), m_dim.out_channels,
                                        rng, mu, sigma);
        }

        void set_param_ptr(ParameterContainer *current_param_cont_ptr) {
            Parameters *par = current_param_cont_ptr->param_list[layer_n];
            current_parameters = dynamic_cast<ConvolutionalParameters *>(par);
        }
        ...
        void forward(const Matrix &prev_layer_data) {
            ...
            internal::convolve_valid(m_dim, prev_layer_data.data(), true, nobs,
                                     current_parameters->m_filter_data.data(), m_z.data()
            );
            ...
            for (int i = 0; i < m_dim.out_channels; i++, channel_start_row += channel_nelem) {
                m_z.block(channel_start_row, 0, channel_nelem, nobs).array() += (current_parameters->m_bias)[i];
            }
        ...
        void backprop(const Matrix &prev_layer_data, const Matrix &next_layer_data) {
            ...
            internal::convolve_full(conv_full_dim, dLz.data(), nobs,
                                    current_parameters->m_filter_data.data(), m_din.data()
            );
        }
        ...
        void update(Optimizer *opt) {
            ...
            AlignedMapVec w(current_parameters->m_filter_data.data(), current_parameters->m_filter_data.size());
        ...
        void update_cw(Optimizer *opt) {
            current_parameters->update_cw(m_df_data, m_db, opt);
        }

        std::vector<Scalar> get_parameters() const {
            std::vector<Scalar> res(current_parameters->m_filter_data.size() + current_parameters->m_bias.size());
            // Copy the data of filters and bias to a long vector
            std::copy(current_parameters->m_filter_data.data(),
                      current_parameters->m_filter_data.data() + current_parameters->m_filter_data.size(),
                      res.begin());
            std::copy(current_parameters->m_bias.data(),
                      current_parameters->m_bias.data() + current_parameters->m_bias.size(),
                      res.begin() + current_parameters->m_filter_data.size());
            return res;
        }

        void set_parameters(const std::vector<Scalar> &param) {
            if (static_cast<int>(param.size()) !=
                current_parameters->m_filter_data.size() + current_parameters->m_bias.size()) {
                throw std::invalid_argument("Parameter size does not match");
            }

            std::copy(param.begin(), param.begin() + current_parameters->m_filter_data.size(),
                      current_parameters->m_filter_data.data());
            std::copy(param.begin() + current_parameters->m_filter_data.size(), param.end(),
        ...
        void aggregate(const Layer &_other) {
            const Convolutional &other = dynamic_cast<const Convolutional &>(_other);
            m_df_data += other.m_df_data;
            m_db += other.m_db;
        }
        ...
        void copy(const Layer &_other) override {
            ...
            const Convolutional &other = dynamic_cast<const Convolutional &>(_other);
            current_parameters->m_filter_data = other.current_parameters->m_filter_data;
...
#endif /* LAYER_CONVOLUTIONALSGD_H_ */
