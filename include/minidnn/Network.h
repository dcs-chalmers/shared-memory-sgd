...
///
    ...
    public:
        ...
        ///
        template<typename DerivedX, typename DerivedY>
        bool fit_parallelizable(Optimizer *opt, const Eigen::MatrixBase<DerivedX> &x,
                                const Eigen::MatrixBase<DerivedY> &y,
                                int batch_size, int epoch, int seed = -1) {
            // We do not directly use PlainObjectX since it may be row-majored if x is passed as mat.transpose()
            // We want to force XType and YType to be column-majored
            ...
            for (int k = 0; k < epoch; k++) {
                ...
                for (int i = 0; i < nbatch; i++) {
                    ...
                    // get latest parameter state. When parallelized, 'this' should
                    // be replaced by the pointer to the reading location
                    std::vector<std::vector<Scalar>> params = this->get_parameters();

                    // update local parameter state to latest parameter state
                    this->set_parameters(params);

                    // compute gradient based on local parameter state
                    this->forward(x_batches[i]);
                    this->backprop(x_batches[i], y_batches[i]);
                    std::vector<std::vector<Scalar>> grad = this->get_derivatives();

                    // now, announce grad somehow
